    ...
    const std::string &value)
{
  ...
  return config_file.substr(0, slash + 1) + value;
}
...
void LoadConfig(const std::string &config_file, hobot::dnn_node::racing_obstacle_detection::PTQYolo5Config &yolo5_config)
{
  ...
  {
    std::string cls_name_file = ResolveConfigPath(
        config_file,
        document["cls_names_list"].GetString());
...
{
  // Shared-memory NV12 path keeps its original ratio mapping.
  float ratio = 1.0;

  // sensor_msgs/Image path uses letterbox and maps boxes back to the
  // original Aurora RGB image with these per-frame values.
  bool use_letterbox_mapping = false;
  float letterbox_scale = 1.0;
  int pad_left = 0;
  int pad_top = 0;
...
{
public:
  ObstacleDetectionNode(const std::string &node_name = "detect",
...
private:
  ...
  std::string config_file_ = "config/config.json";
...
int ObstacleDetectionNode::SetNodePara()
{
  ...
  {
    model_file = ResolveConfigPath(
        config_file_,
        document["model_file"].GetString());
  }
    ...
    const sensor_msgs::msg::Image::ConstSharedPtr img_msg)
{
  ...
  try
  {
    bgr_image = cv_bridge::toCvShare(img_msg, "bgr8")->image;
  }
  catch (const cv_bridge::Exception &e)
  {
    RCLCPP_ERROR(
        rclcpp::get_logger("ObstacleDetectionNode"),
        "Convert Aurora RGB image failed: %s",
        e.what());
    return;
  }
  ...
  dnn_output->use_letterbox_mapping = true;
  dnn_output->original_width = bgr_image.cols;
  dnn_output->original_height = bgr_image.rows;
    ...
    const std::shared_ptr<hobot::dnn_node::DnnNodeOutput> &node_output)
{
  ...
  if (sample_node_output->use_letterbox_mapping)
  {
    const float scale = sample_node_output->letterbox_scale;
    if (scale <= 0.0f ||
        sample_node_output->original_width <= 0 ||
        sample_node_output->original_height <= 0)
    {
      RCLCPP_ERROR(
          rclcpp::get_logger("ObstacleDetectionNode"),
          "Invalid Aurora letterbox mapping values");
      return -1;
    }

    const float max_x =
        static_cast<float>(sample_node_output->original_width - 1);
    const float max_y =
        static_cast<float>(sample_node_output->original_height - 1);

    for (auto &target : pub_data->targets)
    {
      for (auto &roi : target.rois)
      {
        const float model_x1 = static_cast<float>(roi.rect.x_offset);
        const float model_y1 = static_cast<float>(roi.rect.y_offset);
        const float model_x2 = model_x1 + static_cast<float>(roi.rect.width);
        const float model_y2 = model_y1 + static_cast<float>(roi.rect.height);

        float x1 =
            (model_x1 - static_cast<float>(sample_node_output->pad_left)) /
            scale;
        float y1 =
            (model_y1 - static_cast<float>(sample_node_output->pad_top)) /
            scale;
        float x2 =
            (model_x2 - static_cast<float>(sample_node_output->pad_left)) /
            scale;
        float y2 =
            (model_y2 - static_cast<float>(sample_node_output->pad_top)) /
            scale;
        ...
        const int mapped_x1 = static_cast<int>(std::round(x1));
        const int mapped_y1 = static_cast<int>(std::round(y1));
        const int mapped_x2 = static_cast<int>(std::round(x2));
        const int mapped_y2 = static_cast<int>(std::round(y2));

        roi.rect.x_offset = mapped_x1;
        roi.rect.y_offset = mapped_y1;
        roi.rect.width = std::max(0, mapped_x2 - mapped_x1);
        roi.rect.height = std::max(0, mapped_y2 - mapped_y1);
      }
    }
  }
  else if (sample_node_output->ratio != 1.0)
  {
